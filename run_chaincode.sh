...
kubectl exec utility-pod -n dscf -- sh -c "mkdir -p /go/src/$CHAINCODE_FOLDER"

# comment this line if chaincode is in real $GOPATH
export GOPATH=${ROOT_FOLDER}/chaincode

kubectl cp $GOPATH/src/$CHAINCODE_FOLDER utility-pod:/go/src -n dscf
kubectl exec utility-pod -n dscf -- sh -c "ls -lah /go/src/$CHAINCODE_FOLDER"

echo ""
echo "=> RUN_CHAINCODE: Install dependencies for $CHAINCODE_NAME"
# kubectl exec utility-pod -n dscf -- sh -c "cd /go/src/$CHAINCODE_FOLDER && go mod vendor"
...
kubectl exec utility-pod -n dscf -- sh -c "CORE_PEER_LOCALMSPID=Org1MSP CORE_PEER_ADDRESS=org1-peer1:7051 CORE_PEER_TLS_ROOTCERT_FILE=/crypto/org1/peers/org1-peer1/tls/ca.crt CORE_PEER_TLS_ENABLED=true peer lifecycle chaincode install /tmp/${CHAINCODE_NAME}_${CHAINCODE_VERSION}.tar.gz"
...
rm -f pid.txt
kubectl cp utility-pod:/tmp/pid.txt pid.txt -n dscf
PID=$(cat pid.txt)
...
sleep 5
kubectl exec utility-pod -n dscf -- sh -c "CORE_PEER_LOCALMSPID=Org1MSP CORE_PEER_MSPCONFIGPATH=/crypto/org1/users/Admin@org1/msp peer chaincode query -C ${CHANNEL_NAME} -n ${CHAINCODE_NAME} -c '{\"function\":\"queryCar\",\"Args\":[\"CAR1\"]}'"